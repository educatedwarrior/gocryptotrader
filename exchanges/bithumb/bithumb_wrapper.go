...
func (b *Bithumb) CancelExchangeOrder(order exchange.OrderCancellation) (bool, error) {
	_, err := b.CancelTrade(order.Side.Format(b.Name), order.OrderID, order.CurrencyPair.FirstCurrency.String())
	if err != nil {
		return false, err
	}
	return true, err
